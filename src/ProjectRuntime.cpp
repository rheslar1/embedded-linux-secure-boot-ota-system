...
ProjectProfile demoProfile() {
  return ProjectProfile{
      "embedded-linux-secure-boot-ota-system",
      ...
      MetricSpec{"image_size_kib", "KiB", 480.0, true},
      MetricSpec{"rollback_counter_age", "count", 0.0, true},
      true};
}

RuntimeSample nominalSample() {
  return RuntimeSample{1U, 216.0, 0.0, true, "host-ci-nominal"};
}

RuntimeSample unsafeSample() {
  return RuntimeSample{2U, 648.0, 2.0, true, "host-ci-limit-test"};
}

RuntimeSample offlineSample() {
  return RuntimeSample{3U, 216.0, 0.0, false, "host-ci-offline"};
}
