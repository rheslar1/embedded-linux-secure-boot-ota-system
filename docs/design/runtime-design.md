...
1. Build a runtime profile for `embedded-linux-secure-boot-ota-system`.
...
The runtime must make this focus measurable through logs, reports, tests, diagrams, or hardware captures. The proof point for review is: Device lifecycle security, signed kernel execution, fail-safe OTA updates, and recoverable embedded Linux fleet deployment.
